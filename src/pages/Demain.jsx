...
export default function Demain({ onBack }) {
  ...
  useEffect(() => { fetchDemain() }, [])

  async function fetchDemain() {
    setLoading(true)
    ...
    setSeances(avecPresences)
    setLibres(avecBookings)
    setLoading(false)
  }
  ...
  return (
    <div style={{ fontFamily: "'Georgia', serif", background: COLORS.beigeLight, minHeight: '100vh' }}>
      ...
      <main style={{ maxWidth: '700px', margin: '0 auto', padding: '2rem 1rem' }}>
        <h2 style={{ color: COLORS.navy, fontSize: '1.5rem', marginBottom: '0.3rem' }}>🐴 Les cours de demain</h2>
        {dateAffichee && <p style={{ color: COLORS.textLight, marginBottom: '2rem', textTransform: 'capitalize' }}>{dateAffichee}</p>}

        {loading && <p style={{ color: COLORS.textLight }}>Chargement...</p>}

        {!loading && seances.length === 0 && libres.length === 0 && (
          <div style={{ background: 'white', borderRadius: '16px', padding: '2rem', textAlign: 'center', boxShadow: '0 4px 20px rgba(26,39,68,0.06)' }}>
            <p style={{ color: COLORS.textLight, margin: 0 }}>Aucun cours prévu demain.</p>
          </div>
        )}
